...
def main():
    ...
    path = "../../find_params/sdnn_original/" # To be set accordingly 
    spike_times_learn = [path + '/datasets/LearningSet/Face/', path + '/datasets/LearningSet/Motor/']
    #spike_times_learn = [path + '/datasets/TrainingSet/Face/', path + '/datasets/TrainingSet/Motor/']
    ...
    network_params = [{'Type': 'input', 'num_filters': 1, 'pad': (0, 0), 'H_layer': DoG_params['img_size'][1],
                       'W_layer': DoG_params['img_size'][0]},
                      {'Type': 'conv', 'num_filters': 4, 'filter_size': 5, 'th': 6.25},
                      {'Type': 'pool', 'num_filters': 4, 'filter_size': 7, 'th': 0., 'stride': 6},
                      {'Type': 'conv', 'num_filters': 20, 'filter_size': 17, 'th': 20.75},
                      {'Type': 'pool', 'num_filters': 20, 'filter_size': 5, 'th': 0., 'stride': 5},
    ...
    if set_weights:
        weight_path_list = [path_set_weigths + 'weight_' + str(i) + '_' + run_ID +'.npy' for i in range(len(network_params) - 1)]
    ...
    if save_weights:
        ...
        for i in range(len(weights)):
            np.save(path_save_weigths + 'weight_' + str(i) +  '_' + run_ID, weights[i])
    ...
    if save_features:
        np.save(path_features + 'X_train'+ '_' + run_ID, X_train)
        np.save(path_features + 'y_train'+ '_' + run_ID, y_train)
        np.save(path_features + 'X_test'+ '_' + run_ID, X_test)
        np.save(path_features + 'y_test'+ '_' + run_ID, y_test)
    ...
    print('Train Score: '+ run_ID + '   ' + str(train_score)+'\n'+'Test Score: '+ run_ID + '   ' + str(test_score))
