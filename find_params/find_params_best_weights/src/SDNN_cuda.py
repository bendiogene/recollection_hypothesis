...
# --------------------------- Propagation functions ------------------------#
    ...
    def train_features(self):
        ...
        print(f'ITER_TRAIN: {self.ITER_TRAIN}, APLUS: {self.APLUS},AMINUS: {self.AMINUS},TR_learning: {self.TR_learning},SPIKES_T0_CONSIDER: {self.SPIKES_T0_CONSIDER},POOLING_W: {self.POOLING_W},RANDOM_INIT: {self.RANDOM_INIT},TR_L1: {self.TR_L1},TR_L2: {self.TR_L2},CLASS_TR: {self.CLASS_TR}\n{train_score_str} \n{test_score_str}\n{train_score_str_pot}\n{test_score_str_pot}\n')
        
        scores_per_image = {'train':classes_scores_train,'test':classes_scores_test}
        scores_per_image_POT = {'train':classes_scores_train_POT,'test':classes_scores_test_POT}
        
        filename = f'{self.ITER_TRAIN}_{self.APLUS}_{self.AMINUS}_{self.TR_learning}_{self.SPIKES_T0_CONSIDER}_{self.POOLING_W}_{self.RANDOM_INIT}_{self.TR_L1}_{self.TR_L2}_{self.CLASS_TR}.pkl'
        with open(os.path.join("/home/atomassi/find_params/scores_classes_best/",filename),'wb') as f:
          pickle.dump(scores_per_image,f)
          
        filename = f'{self.ITER_TRAIN}_{self.APLUS}_{self.AMINUS}_{self.TR_learning}_{self.SPIKES_T0_CONSIDER}_{self.POOLING_W}_{self.RANDOM_INIT}_{self.TR_L1}_{self.TR_L2}_{self.CLASS_TR}_POT.pkl'
        with open(os.path.join("/home/atomassi/find_params/scores_classes_best/",filename),'wb') as f:
          pickle.dump(scores_per_image_POT,f)
        
        scores_time_slot = {'train':scores_time_train,'test':scores_time_test}
        filename = f'{self.ITER_TRAIN}_{self.APLUS}_{self.AMINUS}_{self.TR_learning}_{self.SPIKES_T0_CONSIDER}_{self.POOLING_W}_{self.RANDOM_INIT}_{self.TR_L1}_{self.TR_L2}_{self.CLASS_TR}.pkl'
        with open(os.path.join("/home/atomassi/find_params/scores_time_best/",filename),'wb') as f:
          pickle.dump(scores_time_slot,f)
