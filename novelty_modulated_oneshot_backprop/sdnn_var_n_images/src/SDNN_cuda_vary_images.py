...
# --------------------------- Initialisation functions ------------------------#
    ...
    def gen_iter_paths(self, path_list, n_images_to_consider = 99999999):
        ...
        #print(len(list(paths_iter)), labels)
        #exit(0)
...
# --------------------------- Propagation functions ------------------------#
    ...
    def train_features(self):
        ...
        for i in set(self.y_train):
          self.class_matrix[i] = np.random.normal(self.RANDOM_INIT,self.std_init,last_layer_shape)
        ...
        for iter_train in range(self.ITER_TRAIN):
          ...
          for img_n in range(self.num_img_train):
              #######################
              # Zied 2: remember
              if img_n>4 and firstt==1:
                  a_plus=a_plus/factor_x
                  a_minus=a_minus/factor_x
                  firstt=0
              #######################
              self.reset_layers()  # Reset all layers for the new image
              if self.DoG:
        ...
        #Zied to fix works only for case 2 images
        train_images_str=f"Images: image1: {train_images[0]} image2: {train_images[1]}"
        ...
        test_score_str = f"test_score: {correct/float(self.num_img_test)}"
        test_score_str_pot = f"weighted_potential_test: {correct_pot/float(self.num_img_test)}"
        
        print(f'n_train_images {self.n_train_images} weights_id {self.weights_ID} w_factor {self.w_factor} rand_init {self.std_init} {test_score_str} {test_score_str_pot}\n{train_images_str}\n')
