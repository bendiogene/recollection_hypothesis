...
def main():
    ...
    # Results directories
    #ZIed: to change here with path to best weights parameters
    ...
    #print(sys.argv[1:])
    #exit(0)
    ITER_TRAIN, APLUS, AMINUS, TR_learning, SPIKES_T0_CONSIDER, POOLING_W, RANDOM_INIT, TR_L1, TR_L2, CLASS_TR = sys.argv[1:]
    # set parameters
    first_net.ITER_TRAIN = int(ITER_TRAIN)
    first_net.APLUS=float(APLUS)
    first_net.AMINUS=float(AMINUS)
    first_net.TR_learning = int(TR_learning)
    first_net.SPIKES_T0_CONSIDER=int(SPIKES_T0_CONSIDER)
    first_net.POOLING_W = int(POOLING_W)
    if RANDOM_INIT == 'T' or RANDOM_INIT == 'F':
      first_net.RANDOM_INIT=RANDOM_INIT
    else:
      first_net.RANDOM_INIT=float(RANDOM_INIT)
      
    first_net.TR_L1=float(TR_L1)
    first_net.TR_L2=float(TR_L2)
    first_net.CLASS_TR=float(CLASS_TR)
